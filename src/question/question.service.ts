...
export class QuestionService {
    ...
    async get(theme: string, language: string, passport: string): Promise<{list: DefaultQuestion[]}>{
        ...
        if(isAutoGenerated && remainCount > 0){
            const integration = await this.themeService.getIntegration(theme);
            const generated: DefaultQuestion[] | null = await this.generator.getQuestion(theme, remainCount, QuestionLevel.easy, language, passport, integration || undefined);
    ...
    async createGenerationPattern(patternDto: GenerationPatternDTO, language: string, passport: string){
        ...
        try{
            ...
            const isCreated = await this.generator.createPattern(patternDto, language, await this.themeService.getIntegration(theme) || undefined);
